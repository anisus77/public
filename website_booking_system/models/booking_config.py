...
class BookingSlot(models.Model):
    ...
    def create(self,vals):
        for value in vals:
            ...
            time_slot = value.get('time_slot_id')
            plan_id = value.get('plan_id')
            saved_data = self.env['booking.slot'].search([('slot_config_id','=',value.get('slot_config_id')),('time_slot_id','=',time_slot),('plan_id','=',plan_id)])
            if len(saved_data)>0:
                raise UserError(_('Record already exist for the selected booking slot & plan.'))
        return super(BookingSlot,self).create(vals)


    @api.depends('time_slot_id', 'plan_id')
    def _compute_display_name(self):
        for rec in self:
            name = rec.time_slot_id.display_name + ',' + rec.plan_id.name
            rec.display_name = name
