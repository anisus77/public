...
class WebsiteSale(WebsiteSale):
    ...
    def cart_update(self, product_id, add_qty=1, set_qty=0, **kw):
        ...
        if bk_plan:
            ...
            order_line = sale_order.order_line.filtered(lambda l: l.product_id.id == int(product_id))[-1]
    ...
    @http.route(['/shop/cart/delete'], type='json', auth="public", methods=['POST'], website=True, csrf=False)
    def cart_del(self,**kw):
        search_sol = request.env['sale.order.line'].browse(kw['line_id'])
        data = search_sol.sudo().unlink();
...
class BookingReservation(http.Controller):
    ...
    def booking_reservation_update_slots(self,**post):
        ...
        return request.env['ir.qweb']._render('website_booking_system.booking_modal_bk_slots_n_plans_div',values)
    ...
    def booking_reservation_slot_plans(self,**post):
        ...
        if sel_date:
            ...
            return request.env['ir.qweb']._render('website_booking_system.booking_sel_plans',values)
        return False
    ...
    def booking_reservation_modal(self,**post):
        ...
        return request.env['ir.qweb']._render('website_booking_system.booking_and_reservation_modal_temp',values)
    ...
    def booking_reservation_modal_update(self,**post):
        ...
        return request.env['ir.qweb']._render('website_booking_system.booking_modal_bk_slots_main_div',values)
    ...
    def booking_reservation_cart_validate(self, **post):
        product_id = post.get('product_id', False)
        sale_order = request.website.sale_get_order()
        search = request.env['booking.slot'].search([('id','=',post.get("wk_plan_id"))])
        plan_id = search.plan_id
        wk_date = datetime.strptime(post.get('wk_date'),'%Y-%m-%d')
        if sale_order:
            # order_line = sale_order.order_line.filtered(lambda l: l.product_id.product_tmpl_id.id == int(product_id))
            order_line = sale_order.order_line.filtered(lambda l: l.product_id.product_tmpl_id.id == int(product_id) and l.booked_slot_id.id == int(post.get('wk_slot_id',-1)) and l.booking_date == wk_date.date() and l.booked_plan_id == plan_id)
