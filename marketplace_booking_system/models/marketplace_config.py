...
class MarketplaceConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        mp_auto_timeslot_approve = self.env['ir.default'].sudo()._get('res.config.settings', 'mp_auto_timeslot_approve')
        mp_auto_plan_approve = self.env['ir.default'].sudo()._get('res.config.settings', 'mp_auto_plan_approve')
        res.update({
