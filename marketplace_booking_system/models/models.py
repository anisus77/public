...
class BookingTimeSlot(models.Model):
    ...
    auto_timeslot_approve = fields.Boolean(string="Auto Timeslot Approve",
        default=lambda self: self.env['ir.default']._get('res.config.settings', 'mp_auto_timeslot_approve'))
    ...
    def fields_view_get(self, view_id=None, view_type='tree', toolbar=False, submenu=False):
        ...
        officer_group = self.env['ir.model.data']._xmlid_lookup(
            'odoo_marketplace.marketplace_officer_group')[2]
        ...
        if self._context.get('mp_new_booking') and officer_group not in groups_ids:
            for node in doc.xpath("//tree//field[@name='start_time']"):
                node.set("readonly", "state not in ['new'] or marketplace_seller_id != %s" % str(self.env.user.partner_id.id))
...
class BookingPlan(models.Model):
    ...
    auto_plan_approve = fields.Boolean(string="Auto Plan Approve",
        default=lambda self: self.env['ir.default']._get('res.config.settings', 'mp_auto_plan_approve'))
    ...
    def plan_auto_approve(self):
        auto_plan_approve = self.env['ir.default']._get('res.config.settings', 'mp_auto_plan_approve')
        for obj in self:
