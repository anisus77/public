...
class SaleOrder(models.Model):
    ...
    # for rental contract smart button
    def action_view_rental_contract(self):
        contract_id = self.order_line.rental_contract_ids
        return {
            'type':'ir.actions.act_window',
            'name':'Rental Contracts',
            'res_model':'rental.order.contract',
            'domain':[('id','=',contract_id.ids)],
            'view_mode':'tree,form',
    ...
    def create_deposit_amt_invoice(self):
        ...
        security_refund_product_id = self.env['ir.default'].sudo()._get('res.config.settings',
            ...
            invoice_vals['invoice_line_ids'].append((0, 0,
                {
                    ...
                    'display_type': sol.display_type or 'product',
        ...
        for rec in self:
            ...
            if security_refund_product_id:
                ...
                inv_line_res = {
                        ...
                        'tax_ids': [(6, 0, security_refund_product_obj.taxes_id.ids if security_refund_product_obj.taxes_id else [])],
                        "rental_contract_id": (rec.current_rental_contract_id and rec.current_rental_contract_id.id) or rec.rental_contract_ids[0].id,
...
class SaleOrderCancel(models.TransientModel):
    ...
    def action_cancel(self):
        if self.order_id.order_line.rental_state:
            self.order_id.order_line.rental_state = 'cancel'
